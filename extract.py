...
(잡알리오는 자격요건·우대사항 본문이 있어 본문에서 찾고, 사람인은 제목·키워드만 쓴다. 추정 항목은 30건 손 대조로 정확도 확인)
...
SKILLS = ["SQL", "Python", "Tableau", "Power BI", "Excel", "엑셀", "Amplitude", "GA4", "Google Analytics",
          "BigQuery", "Spark", "Hadoop", "Looker", "Airflow", "통계", "머신러닝", "AI", "데이터", "정보처리기사",
          "컴퓨터활용능력", "SQLD", "ADsP", "빅데이터분석기사"]
NEW_COLS = ["신입가능", "학력요건", "졸업예정가능", "요구스킬", "고용형태"]


def enrich(r):
    text = f"{r.get('title','')} {r.get('keyword','')} {r.get('자격요건원문','')} {r.get('우대사항원문','')}"
    ...
    if re.search(r"졸업\s*예정", text):
        r["졸업예정가능"] = "Y"
    elif re.search(r"기졸업|졸업자만|졸업자에 한|졸업한 자|학위 취득자|학위를 취득한", text):
        r["졸업예정가능"] = "N"
    else:
        r["졸업예정가능"] = "Y"
